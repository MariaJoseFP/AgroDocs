  ...
  if (archivo) {
    ...
    lector.onload = function (e) {
      ...
      if (!contieneParentesis(datos)) {
        ...
        alert("El documento contiene parentesis que no son admitidos. Se eliminaran.");
  ...
  $.ajax({
    ...
    success: function (data) {
      if (data.status == "Éxito") {
        ...
        var mensaje = data.status;
        if (mensaje.slice(0, 15) == 'Duplicate entry') {
          var mensaje = "El número de traslado ya existe en la base de datos.";
        } else {
           var mensaje = "No se pudo guardar en la base de datos. " + data.insert;
        }
      //  alert(mensaje);
        alert(data.status);
      }
...
function autocompletarCliente() {
  ...
  var otroCampo = document.getElementById('otroCliente');
  if (cliente !== '') {
    $.ajax({
      type: "GET",
...
function autocompletarRemolq() {
  var remolq = $("#Remol").val();
  remolq = $("#Remol").val();
  console.log(remolq)
  // Verifica si no se puede leer el valor de la primera ubicación
  if (remolq === undefined || remolq === null) {
    // Intenta leer el valor de la segunda ubicación
    remolq = $("#Remolq").val();
  }
  ...
  if (unidad !== '') {
    $.ajax({
      ...
      success: function (data) {
        ...
        if (respuesta.tipo_unidad === "TORTON" || respuesta.tipo_unidad === "RABON" || respuesta.tipo_unidad === "RABÓN") {
          $(".campos_remolq").css({
            "display": "none",
          });
        } else {
...
function guardarTraslado() {
  ...
  var placas_plana = document.getElementById('placas_plana').value;
  var clienteAdicional = $("#clienteAdicional").val();
  if (
    traslado === "" ||
    cliente === "" ||
    pollos === "" ||
    ...
    operador === "" ||
    unidad === "" ||
    (cliente === "OTRO" && clienteAdicional === "") ||
    (tipo_unidad === "TRACTOCAMIÓN" && placas_plana === "")
  ) {
...
function guardarCambios() {
  ...
  if (Traslado === '') {
    ...
    $.ajax({
      ...
      method: 'POST',
      contentType: 'application/json',
      data: JSON.stringify({ datosTraslado: jsonData }),
      success: function (data) {
        if (data.status == "Éxito") {
          alert("Los datos se han guardado correctamente.");
          location.reload(true);
...
document.getElementById('btnDescargar').addEventListener('click', function () {
  ...
  var nombreArchivo = 'formato_programacion.xlsx';

  // URL del servicio en el servidor que proporciona el archivo
  var urlServicio = 'documentos/' + nombreArchivo; // Reemplaza esto con la URL correcta en tu servidor

  // Crear un enlace oculto para la descarga
  var enlaceDescarga = document.createElement('a');
  enlaceDescarga.href = urlServicio;
...
function cancelarTraslado(btn) {
  var resultado = confirm("¿Estas seguro de que deseas cancelar el traslado?");

  if (resultado) {
    var id = btn.getAttribute('id');
    // Realiza la solicitud AJAX
    $.ajax({
      url: 'php/cancelarTraslado.php',
      type: 'GET',
      data: { traslado: id },
      success: function (response) {
...
function verBD() {
  window.location.href = 'php/bd_lyt.php';
}